...
resource "aws_security_group" "alb" {
  ...
  egress {
    from_port   = 0
    to_port     = 0
    protocol    = "-1" # "-1" na descrição de protocol é todo os protocolos 
    cidr_blocks = ["0.0.0.0/0"]
  }
...
resource "aws_security_group" "autoscaling" {
  ...
  ingress {
    from_port   = 22 #será a porta do ssh para verificar
    to_port     = 22
    protocol    = "tcp"
    cidr_blocks = ["0.0.0.0/0"]
  }
  ...
  ingress {
    from_port       = 80 #abri a porta 80 para o load balancer
    to_port         = 80
    protocol        = "tcp"
